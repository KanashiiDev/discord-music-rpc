...
export async function updateMusicCard() {
  if (musicCardState.musicCardUpdatePending || document.hidden) return;

  const cardContainer = dom.musicCard.container;
  if (!cardContainer || cardContainer.offsetParent === null) return;

  musicCardState.musicCardUpdatePending = true;

  try {
    const response = await fetch("/activity");
    if (!response.ok) throw new Error("Activity fetch failed");

    const { activity: act } = await response.json();
    ...
    console.error("Music Card Update Error:", err);
  } finally {
    musicCardState.musicCardUpdatePending = false;
  }
}
